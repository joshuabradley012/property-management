...
var createEntry = function(entry, tenant, tenantId) {
  ...
  var type = !!entry.type.match(/owner draw/gi) ? 'Owner Draw' : !!entry.type.match(/loan/gi) ? 'Loan' : !!entry.type.match(/overpayment/gi) ? 'Reminbursement' : !!entry.type.match(/late fee/gi) ? 'Late Fee' : entry.type;
  if (type === 'Misc.') {
    type = !!entry.name.match(/overpayment/gi) ? 'Reminbursement' : !!entry.name.match(/owner draw/gi) ? 'Owner Draw' : !!entry.name.match(/late fee/gi) ? 'Late Fee' : entry.name;
  }
